...
## 你必须遵守
1. 日期只用真实存在的 YYYY-MM-DD（不要 2026-09-31 这种）；时间是北京时间 +08:00。
2. 开放起点是 2026-09-15（含），无结束日，可跨年。填盒子前先 next_empty_dates，不要猜日期。
3. 每个日期只能有 1 个盒子。
4. 不能创建过去的日期（早于北京时间「今天」）；也不能早于 2026-09-15。
...
12. 写多天：先 next_empty_dates(count=N)，再一次 create_box 传入 N 个不同日期。不传 count 默认返回 90 个空位；count 最大 366。
13. calendar_status：start_date 不能晚于 end_date；失败时仍是工具成功返回，正文 { ok:false, error, code }（与批量工具一样看正文，不是 Step error）。
14. year_stats = start_date 所在年的统计（2026 年从 2026-09-15 起算；之后整年）；stats 才对应当前查询范围。
...
## 常见操作顺序
写一批新盒子：
1) next_empty_dates（需要几个就传 count；不传默认 90 个）
...
{
  "boxes": [
    { "date": "2026-09-15", "content": "开放日留言。", "prompt": "可选备注" },
    { "date": "2026-09-16", "content": "只有正文也可以，不传 prompt。" }
  ]
}
...
出参示例：
{
  "results": [
    ...
    { "date": "2026-09-16", "ok": true, "status": "locked" }
  ]
}

失败条示例：
{ "date": "2026-09-14", "ok": false, "error": "不能早于开放起点 2026-09-15", "code": "DATE_BEFORE_START" }

### update_box
入参示例：
{
  "boxes": [
    { "date": "2026-09-15", "content": "改后的正文", "prompt": "可选；不传则 prompt 保持原值" },
    { "date": "2026-09-99", "content": "坏日期只会让这一条失败" }
  ]
}
...
出参示例：
{
  "results": [
    { "date": "2026-09-15", "ok": true, "status": "locked" },
    { "date": "2026-09-99", "ok": false, "error": "日期格式必须是 YYYY-MM-DD，且必须是真实存在的日历日", "code": "INVALID_DATE" }
  ]
}
...
{
  "results": [
    { "date": "2026-09-16", "ok": true, "deleted": true, "message": "盲盒已删除，该日期现在是空盒。" }
  ]
}

### open_box
入参：
{ "dates": ["2027-12-25", "2026-09-15"] }
...
出参示例（未到日 / 已到日）：
{
  "results": [
    ...
    },
    {
      ...
      "content": "开放日留言。",
      "prompt": "可选备注",
      "opened_at": "2026-09-15T08:15:00+08:00"
    }
  ]
}
...
出参成功示例：
{
  ...
  "dates": [
    { "date": "2026-09-15", "status": "locked", "has_box": true }
  ]
}
...
- year_stats = start_date 所在年（2026 从 9/15 起算分母）
...
- DATE_BEFORE_START → 不能早于开放起点 2026-09-15
